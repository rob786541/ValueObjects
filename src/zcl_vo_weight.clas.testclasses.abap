...
CLASS ltcl_weight IMPLEMENTATION.
  ...
  METHOD wrong_uom.
    TRY.
      ...
      CATCH zcx_value_object INTO e.
        cl_abap_unit_assert=>assert_equals( exp = 'UOM M is not a valid weight dimension'
                                            act = e->get_text( ) ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD negative.
    TRY.
        ...
        cl_abap_unit_assert=>fail( ).
      CATCH zcx_value_object INTO e.
        cl_abap_unit_assert=>assert_equals( exp = 'Weight -12 is not valid'
                                            act = e->get_text( ) ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD rounded4.
    TRY.
        cut = NEW #( i_weight = 1553
                     i_uom    = NEW #( 'G' ) ).
        cl_abap_unit_assert=>assert_equals( exp = 1600
                                            act = cut->get_weight_rounded( i_uom       = NEW #( 'g' )
                                                                           i_precision = 2 ) ).
      CATCH zcx_value_object.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ...
  METHOD to_string4.
    TRY.
        ...
        cl_abap_unit_assert=>assert_equals( exp = '0,015 TO'
                                            act = cut->to_string_with_uom( i_uom = NEW #( 'to' ) ) ).
      CATCH zcx_value_object.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD to_string5.
    TRY.
        ...
        cl_abap_unit_assert=>assert_equals( exp = ''
                                            act = cut->to_string_empty_for_zero( i_uom = NEW #( 'to' ) ) ).
      CATCH zcx_value_object.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD equal1.
    TRY.
        cut = NEW #( i_weight = 15
                     i_uom    = NEW #( 'kg' ) ).
        DATA(second) = NEW zcl_vo_weight( i_weight = 15
                                          i_uom    = NEW #( 'kg' ) ).
        cl_abap_unit_assert=>assert_true( cut->is_equal( second ) ).
      CATCH zcx_value_object.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD equal2.
    TRY.
        ...
        DATA(second) = NEW zcl_vo_weight( i_weight = 15
                                          i_uom    = NEW #( 'g' ) ).
        cl_abap_unit_assert=>assert_false( cut->is_equal( second ) ).
      CATCH zcx_value_object.
        cl_abap_unit_assert=>fail( ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD equal3.
    TRY.
        cut = NEW #( i_weight = 1
                     i_uom    = NEW #( 'kg' ) ).
        DATA(second) = NEW zcl_vo_weight( i_weight = 1000
                                          i_uom    = NEW #( 'g' ) ).
        cl_abap_unit_assert=>assert_true( cut->is_equal( second ) ).
      CATCH zcx_value_object.
        cl_abap_unit_assert=>fail( ).
  ...
  METHOD equal4.
    TRY.
        cut = NEW #( i_weight = 15
                     i_uom    = NEW #( 'g' ) ).
        DATA(second) = NEW zcl_vo_weight( i_weight = '0.00001653468315137833118749889768778991'
                                          i_uom    = NEW #( 'ton' ) ).
        cl_abap_unit_assert=>assert_true( cut->is_equal( second ) ).
      CATCH zcx_value_object.
        cl_abap_unit_assert=>fail( ).
